...
async function LCdeleteComment(id) {
    ...
    try {
        ...
        const response = await fetch(`/comentario/delete/${id}`, {
            method: 'DELETE',
        });
        //  mostrar resultado
...
async function readComments() {
    ...
    try {
        ...
        console.error("Error:", error);
        LCdisplayMessage("Erro ao ler comentarios");
    }
    //  retornar
    return (users);
} // end readUsers ( )
...
async function deleteForm(id) {
        ...
        try {
            const response = await fetch(`/formulario/delete/${id}`, { 
                method: 'DELETE',
            });
    ...
    try {
     // Remover o endereço
       ...
       // console.log('Endereço resultado:', resultEndereco);
        let comentario = [ ];
        comentario = await readComments();

       for(let i = 0; i < comentario.length; i++)
       {
            if(comentario[i].id_pessoa == id)
            {
               await LCdeleteComment(comentario[i].id);
            }
       }

       let formulario = [ ];
       formulario = await getAllForms();

       for(let i = 0; i < formulario.length; i++)
        {
             if(formulario[i].id_pessoa == id)
             {
                await deleteForm(formulario[i].id_formulario);
             }
        }
